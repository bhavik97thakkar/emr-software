...
import { Search, Phone, ChevronRight, UserPlus, Filter, CreditCard, Stethoscope } from 'lucide-react';
             ...
             return (
               <Link key={p.mobile} to={`/patient/${p.mobile}`} className="flex items-center justify-between px-6 py-5 hover:bg-slate-50 transition-all group">
                 <div className="flex items-center space-x-5">
                    ...
                    <div>
                       <p className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors leading-none">{p.name}</p>
                       <div className="flex items-center space-x-4 mt-2 text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                          <span className="flex items-center"><Phone size={10} className="mr-1.5 opacity-40" /> {p.mobile}</span>
