...
const RESET_TOKEN = import.meta.env.VITE_RESET_TOKEN;
...
export const DB = {
  ...
  async resetTenantOnCloud() {
    try {
      const token = localStorage.getItem('token');
      if (!token) throw new Error('Not authenticated');
      if (!RESET_TOKEN) throw new Error('Reset token not configured');

      const res = await fetch(`${API_BASE_URL}/admin/reset-tenant`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'X-Reset-Token': String(RESET_TOKEN),
        },
      });
      const data = await res.json();
      return !!data.success;
  ...
  async resetClinicFresh() {
    const ok = await this.resetTenantOnCloud();
    if (!ok) return false;
